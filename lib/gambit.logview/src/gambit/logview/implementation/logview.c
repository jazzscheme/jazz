/* File: "xlog.c"   (c) 1997-1998, Marc Feeley */

/* Tool to examine state logs on an X11 display or Mac */

/* To compile:  gcc -O -o xlog xlog.c -L /usr/X11R6/lib -lX11 */
...
{ Rect r;
  ...
  win->win = NewWindow(NULL, &r, "\pxlog", 1, noGrowDocProc, -1L, 1, 0);
  if (win->win == NULL)
  { fprintf( stderr, "can't allocate window\n" );
    exit (1);
  }
...
{ if (win->ps_file == NULL)
  { win->ps_file = fopen( name, "w" );
    ...
    { int i;
      ...
      fprintf( win->ps_file, "%%%%Creator: xlog\n" );
      if (for_printing)
      ...
      fprintf( win->ps_file, "/$XLOGDict 32 dict def $XLOGDict begin\n" );
      if (!for_printing) fprintf( win->ps_file, "%%" );
      fprintf( win->ps_file, "/$XLOGEnd {showpage end} def %%%% [ 0 -1 1 0 0 0 ] concat -750 17 translate 0.72 0.72 scale\n" );
      fprintf( win->ps_file, "100 45 currenthalftone\n" );
      fprintf( win->ps_file, "%1.6f %1.6f scale\n", zoom, zoom );
      if (for_printing) fprintf( win->ps_file, "%%" );
      fprintf( win->ps_file, "/$XLOGEnd {$XLOGState restore end} def /$XLOGState save def\n" );
      fprintf( win->ps_file, "/$XLOGColorR [ " );
      for (i=0; i<win->nbcolors+OTHER; i++)
        fprintf( win->ps_file, "%1.2f ", RED(win_get_color(win, i))/255.0 );
      fprintf( win->ps_file, "] def\n" );
      fprintf( win->ps_file, "/$XLOGColorG [ " );
      for (i=0; i<win->nbcolors+OTHER; i++)
        fprintf( win->ps_file, "%1.2f ", GREEN(win_get_color(win, i))/255.0 );
      fprintf( win->ps_file, "] def\n" );
      fprintf( win->ps_file, "/$XLOGColorB [ " );
      for (i=0; i<win->nbcolors+OTHER; i++)
        fprintf( win->ps_file, "%1.2f ", BLUE(win_get_color(win, i))/255.0 );
      fprintf( win->ps_file, "] def\n" );
      fprintf( win->ps_file, "/$XLOGColorK [ " );
      for (i=0; i<win->nbcolors+OTHER; i++)
        {
          RGB rgb = win_get_color(win, i);
      ...
      fprintf( win->ps_file, "/$XLOGSetcolor {" );
      fprintf( win->ps_file, "dup $XLOGColorR exch get exch " );
      fprintf( win->ps_file, "dup $XLOGColorG exch get exch " );
      fprintf( win->ps_file, "    $XLOGColorB exch get      setrgbcolor} def\n" );
      if (!graph_black_and_white) fprintf( win->ps_file, "%%" );
      fprintf( win->ps_file, "/$XLOGSetcolor {$XLOGColorK exch get setgray} def\n" );
      fprintf( win->ps_file, "/$XLOGBox {$XLOGSetcolor 0.75 add moveto dup 0 exch rlineto exch 0 rlineto neg 0 exch rlineto closepath fill} def\n" );
      fprintf( win->ps_file, "/$XLOGLeft {$XLOGSetcolor 2.25 sub moveto show} def\n" );
      fprintf( win->ps_file, "/$XLOGRight {$XLOGSetcolor 2.25 sub moveto dup stringwidth pop neg 0 rmoveto show} def\n" );
      fprintf( win->ps_file, "/$XLOGCenter {$XLOGSetcolor 2.25 sub moveto dup stringwidth pop 2 div neg 0 rmoveto show} def\n" );
...
{ if ((w > 0) && (h > 0))
    if (win->ps_file == NULL)
      ...
      fprintf( win->ps_file, "%d %d %d %d %d $XLOGBox\n", w, h, x, y, color );
}
...
{ if (win->ps_file == NULL)
  ...
  { fprintf( win->ps_file, "(%s) %d %d %d ", str, x, y, color );
    switch (where)
    { case CENTER: fprintf( win->ps_file, "$XLOGCenter\n" ); break;
      case LEFT:   fprintf( win->ps_file, "$XLOGLeft\n" );   break;
      case RIGHT:  fprintf( win->ps_file, "$XLOGRight\n" );  break;
    }
  }
}
...
char *name;
{
  ...
  do
    {
      sprintf (filename, "%s.log%03d", name, log_nb_traces);
      f = fopen (filename, "rb");
      if (f == NULL)
        break;
      read_log_file (filename, f);
      fclose (f);
      total_nb_events += log_trace[log_nb_traces].len;
      log_nb_traces++;
    } while (1);
  ...
  time_unit_name = "sec";
  time_unit_scale = 6-compression_scale;

  { int i, j;
    U32 compression = 1;
    for (i=0; i<compression_scale; i++) compression *= 10;
    for (i=0; i<log_nb_traces; i++)
      for (j=log_trace[i].len-1; j>=0; j--)
      {
        log_trace[i].event[j].time =
          U64_div_U32 (U64_sub_U64 (log_trace[i].event[j].time, log_min_time),
                       compression);
      }
    log_min_time = U64_div_U32 (log_min_time, compression);
    log_max_time = U64_div_U32 (log_max_time, compression);
    log_max_time = U64_sub_U64 (log_max_time, log_min_time);
  }
}
...
{ int b = LABEL_THEIGHT+HIST_THEIGHT+PERC_THEIGHT+PLOT_THEIGHT+TRACES_THEIGHT;
  ...
  sprintf( title, "program = \"%s\"  p = %d", log_filename, log_nb_traces );
...
{ int b = LABEL_THEIGHT+HIST_THEIGHT+PERC_BORDER-1;
  ...
  { int event_num = xlate[i];
    ...
    j += k;
    if (print_statistics)
      printf ("state=%d percent=%d\n", event_num, k*100/full_plot_width);
  }
}
...
{ char command[100];
  ...
  char *p1, *p2;
  if (send_to_printer) p1 = "xlog"; else p1 = log_filename;
  p2 = postscript_file;
  while (*p1 != '\0') *p2++ = *p1++;
...
{ int i;
  ...
  if (!(arg < ac))
  { fprintf( stderr, "usage: xlog [options] program_name\n\n" );
