      ...
      (lambda (src bin bin-uptodate?)
        (if (or force? (%%not bin) (%%not bin-uptodate?))
            (let ((name (%%path-name src))
              ...
              (let ((manifest-path (jazz.package-manifest-path jazz.build-package name))
                    ;; remove explicit digest passing when gambit bug that forces us to generate a jscm is fixed
                    (manifest-digest (or digest (jazz.path-digest src))))
                (jazz.save-manifest manifest-path (%%make-manifest manifest-name manifest-digest)))))))))
