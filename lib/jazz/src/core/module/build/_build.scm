...
(define (jazz.copy-package package)
  (let* ((name (%%package-name package))
    ...
    (if (and (jazz.file-exists? dst) (>= (jazz.file-modification-time dst) (jazz.file-modification-time src)))
        (jazz.repository-find-package jazz.Install-Repository name)
      (begin
        (jazz.create-directories (jazz.repository-pathname jazz.Install-Repository dir))
        (if (jazz.file-exists? dst)
            (jazz.file-delete dst))
        (jazz.file-copy src dst)
        (let ((package (jazz.load-package jazz.Install-Repository name dst)))
          (%%table-set! (%%repository-packages-table jazz.Install-Repository)
                        name
                        package)
          package)))))
