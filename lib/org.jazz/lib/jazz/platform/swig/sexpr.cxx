 ...
 * sexpr.cxx
 *
 * sexpr language module for SWIG.
 * ----------------------------------------------------------------------------- */
...
void SEXPR::main(int argc, char *argv[]) {
	int i;
	
	SWIG_library_directory("sexpr");
	SWIG_config_file("sexpr.swg");
	for (i = 1; i < argc; i++) {
		if (!Strcmp(argv[i], "-help")) {
			Printf(stdout, "sexpr Options (available with -sexpr)\n");
		}
	}
	f_jazz = NewString("");
...
int SEXPR::constructorHandler(Node *n) {
...
int SEXPR::destructorHandler(Node *n) {
	// Let SWIG generate a global forwarding function.
...
int SEXPR::memberfunctionHandler(Node *n) {
	// Let SWIG generate a global forwarding function.
	Setattr(n, "sexpr:memberfunction", "1");
	return Language::memberfunctionHandler(n);
}



int SEXPR::membervariableHandler(Node *n) {
	// Let SWIG generate a get/set function pair.
	Setattr(n, "sexpr:membervariable", "1");
	return Language::membervariableHandler(n);
}



int SEXPR::functionWrapper(Node *n) {
	...
	if (CPlusPlus) {
		...
		if (Getattr(n, "sexpr:memberfunction"))
			emit_method(n);
		else if (Getattr(n, "sexpr:membervariable")) {
...
void SEXPR::emit_function(Node *n, String *name) {
...
int SEXPR::constantWrapper(Node *n) {
...
int SEXPR::variableWrapper(Node *n) {
...
int SEXPR::classDeclaration(Node *n) {
	return Language::classDeclaration(n);
}



int SEXPR::enumDeclaration(Node *n) {
	String *name = Getattr(n, "sym:name");
...
void SEXPR::emit_struct_union(Node *n, bool un = false) {
	String *name = Getattr(n, "sym:name");
	String *kind = Getattr(n, "kind");
...
void SEXPR::emit_export(Node *n, String *name) {
...
void SEXPR::emit_inline(Node *n, String *name) {
...
String *SEXPR::lispify_name(Node *n, String *ty, const char *flag, bool kw) {
...
String *SEXPR::strip_parens(String *string) {
...
String *SEXPR::infix_to_prefix(String *val, char split_op, const String *op, String *type) {
...
*/
String *SEXPR::convert_literal(String *literal, String *type, bool try_to_split) {
...
String *SEXPR::lispy_name(char *name) {
	bool helper = false;
...
extern "C" Language *swig_sexpr(void) {
	return new SEXPR();
}
