...
(cond-expand
  (gambit
    (define-macro (:optional rest default-value)
      `(if (null? ,rest)
           ,default-value
         (car ,rest))))
  (else
...
(cond-expand
  (gambit
    (define tm:set-time-type! time-type-set!)
    (define tm:set-time-nanosecond! time-nanosecond-set!)
    (define tm:set-time-second! time-second-set!))
  (else
    (define tm:set-time-type! set-time-type!)
    (define tm:set-time-nanosecond! set-time-nanosecond!)
    (define tm:set-time-second! set-time-second!)))
...
(define (tm:get-time-of-day)
  (let* ((time (time->seconds (current-time)))
	 (seconds (inexact->exact (floor (time->seconds (current-time)))))
	 (milliseconds (inexact->exact (floor (* (- time seconds) 1000)))))
    (values seconds milliseconds)))
...
(define (tm:time-difference time1 time2 time3)
  ...
  (tm:set-time-type! time3 time-duration)
  (if (time=? time1 time2)
      (begin
	...
	(tm:set-time-nanosecond! time3 0))
      (receive
       ...
       (tm:set-time-second! time3 secs)
       (tm:set-time-nanosecond! time3 nanos)))
  time3)
...
(define (tm:add-duration time1 duration time3)
  ...
  (if (not (eq? (time-type duration) time-duration))
	...
	(let ((r (remainder nsec-plus tm:nano))
          ...
          ; (tm:set-time-type! time3 (time-type time1))
	  (if (negative? r)
	      (begin
		(tm:set-time-second! time3 (+ sec-plus q -1))
		(tm:set-time-nanosecond! time3 (+ tm:nano r)))
	      (begin
		(tm:set-time-second! time3 (+ sec-plus q))
		(tm:set-time-nanosecond! time3 r)))
	  time3))))
...
(define (tm:subtract-duration time1 duration time3)
  ...
  (if (not (eq? (time-type duration) time-duration))
	...
	(let ((r (remainder nsec-minus tm:nano))
	  ...
	  (if (negative? r)
	      (begin
		(tm:set-time-second! time3 (- sec-minus q 1))
		(tm:set-time-nanosecond! time3 (+ tm:nano r)))
	      (begin
		(tm:set-time-second! time3 (- sec-minus q))
		(tm:set-time-nanosecond! time3 r)))
	  time3))))
...
(define (time-monotonic->time-utc time-in)
  ...
  (let ((ntime (copy-time time-in)))
    (tm:set-time-type! ntime time-tai)
...
(define (time-monotonic->time-utc! time-in)
  ...
  (tm:set-time-type! time-in time-tai)
...
(define (time-utc->time-monotonic time-in)
  ...
  (let ((ntime (tm:time-utc->time-tai! time-in (make-time #f #f #f)
    ...
    (tm:set-time-type! ntime time-monotonic)
    ntime))
...
(define (time-tai->time-monotonic! time-in)
  (if (not (eq? (time-type time-in) time-tai))
      (tm:time-error 'time-tai->time-monotonic!  'incompatible-time-types time-in))
  (tm:set-time-type! time-in time-monotonic)
  time-in)
