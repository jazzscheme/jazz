...
(define (jazz.compile-source-path src #!key (options #f) (cc-options #f) (ld-options #f) (digest #f) (force? #f))
  (let ((options (or options jazz.compile-options))
    ...
    (let ((binpck (and bin (jazz.load-package bin))))
      (if (or force? (%%not binpck) (%%not (jazz.src-determine/cache-identical? src binpck)))
          (let ((name (%%path-name src))
            ...
            (let ((package-path (jazz.path-bin-package src jazz.bin-repository))
