										...
										LPSTACKFRAME * pStackFrame)
{
    ...
    #ifdef _X86_
    g_stFrame.AddrPC.Offset       = pContext->Eip ;
    g_stFrame.AddrPC.Mode         = AddrModeFlat                ;
    g_stFrame.AddrStack.Offset    = pContext->Esp ;
    g_stFrame.AddrStack.Mode      = AddrModeFlat                ;
    g_stFrame.AddrFrame.Offset    = pContext->Ebp ;
    g_stFrame.AddrFrame.Mode      = AddrModeFlat                ;
    #else
    g_stFrame.AddrPC.Offset       = (DWORD)pContext->Fir ;
    g_stFrame.AddrPC.Mode         = AddrModeFlat ;
    g_stFrame.AddrReturn.Offset   =
                                   (DWORD)pContext->IntRa;
    g_stFrame.AddrReturn.Mode     = AddrModeFlat ;
    g_stFrame.AddrStack.Offset    =
                                   (DWORD)pContext->IntSp;
    g_stFrame.AddrStack.Mode      = AddrModeFlat ;
    g_stFrame.AddrFrame.Offset    =
                                   (DWORD)pContext->IntFp;
    g_stFrame.AddrFrame.Mode      = AddrModeFlat ;
    #endif

    return ( InternalGetStackTraceString ( dwOpts , pThread, pContext, pStackFrame ) ) ;
}
										...
										LPSTACKFRAME * pStackFrame  )
{
    ...
    ASSERT ( FALSE == IsBadReadPtr ( pContext                      ,
                                     sizeof ( CONTEXT ) ) ) ;
    if ( TRUE == IsBadReadPtr ( pContext                      ,
                                sizeof ( CONTEXT ) ) )
    {
        TRACE0 ( "GetStackTraceString - invalid pContext!\n" ) ;
        return ( NULL ) ;
    }
                                        ...
                                        UINT                 uiSize   )
{
    ...
    __try
    {
        szRet = GetFirstStackTraceString ( dwOpts , pThread, pContext, NULL ) ;
        if ( NULL == szRet )
        {
            __leave ;
        }
                                       ...
                                       UINT                 uiSize   )
{
    ...
    __try
    {
        szRet = GetNextStackTraceString ( dwOpts , pThread, pContext, NULL ) ;
        if ( NULL == szRet )
        {
            __leave ;
        }
...
LPCTSTR __stdcall GetRegisterString ( CONTEXT * pContext )
{
    // Check the parameter.
    ASSERT ( FALSE == IsBadReadPtr ( pContext                      ,
                                     sizeof ( CONTEXT ) ) ) ;
    if ( TRUE == IsBadReadPtr ( pContext                      ,
                                sizeof ( CONTEXT ) ) )
    {
        TRACE0 ( "GetRegisterString - invalid pContext!\n" ) ;
        return ( NULL ) ;
    }
...
BOOL __stdcall GetRegisterStringVB ( CONTEXT *            pContext ,
                                     LPTSTR               szBuff  ,
                                     UINT                 uiSize   )
{
    ASSERT ( FALSE == IsBadWritePtr ( szBuff , uiSize ) ) ;
    ...
    __try
    {
        szRet = GetRegisterString ( pContext ) ;
        if ( NULL == szRet )
        {
            __leave ;
        }
...
void InitSymEng ( void )
{
    if ( FALSE == g_bSymEngInit )
    {
        ...
        VERIFY ( BSUSymInitialize ( (DWORD)hPID ,
                                    hPID        ,
                                    NULL        ,
                                    // gcartier: this was TRUE but will sometimes make damn Windows
                                    // scan the whole hard drive for modules!!! As we do not use DLL
                                    // in Jazz for the moment I don't think this has any impact...
                                    // ARRRRR putting it to FALSE breaks the StackWalk
                                    TRUE         ) ) ;
        g_bSymEngInit = TRUE ;
    }
}
